...
- Hermes LAN URL: `http://192.168.0.21:9119`
- Hermes upstream health URL: `http://127.0.0.1:9119`
...
The design follows the Dashboard's existing rounded cards and button vocabulary while giving Hermes a recognizable branded surface. On desktop, the model row changes from three equal tracks to `26% / 34% / 40%` for llama.cpp, vLLM, and ETC so vLLM and Hermes receive more room. The approved portrait grows to `104px × 116px` on desktop. The compact toggle sits beside a vertical stack of the two opening buttons. `Open Hermes Local` uses a Canva-like green gradient and green shadow; `Open Hermes Tailscale` uses a distinct cyan-to-blue gradient and blue shadow. Below the existing 1200px breakpoint, the model row keeps its current responsive two-column and one-column behavior.
...
- `GET /api/hermes/status`
  ...
  - Report the fixed LAN URL `http://192.168.0.21:9119`, Tailnet URL, and ports without exposing secrets.
...
`Open Hermes Local` opens the status response's fixed `localUrl` (`http://192.168.0.21:9119`) in a new tab with `noopener,noreferrer`. The existing Caddy configuration already binds that exact LAN address and proxies it to the loopback-only Hermes upstream; this change does not modify Caddy.
...
- Verify the live card reads both fixed Local and Tailscale URLs, shows the approved logo at `104px × 116px`, and renders distinct green and blue vertically stacked opening buttons.
- Verify the desktop model row uses `26% / 34% / 40%` tracks while existing responsive breakpoints remain intact.
- Verify the compact action toggle shows `Stop` whenever either service is active and `Start` only when both are inactive.
- Exercise a live Stop/Start cycle through the Dashboard API and confirm LAN, Tailscale, and loopback health return after Start.
